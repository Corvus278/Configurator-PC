...
def check_compatibility(first_part1, second_part1):
    ...
    # ram
        ...
        if name2 == 'motherboard':
            if int(part1.get('die_count')) > int(part2.get('ram_count')):
                return False
            ...
            if int(part1.get('frequency')) > int(part2.get('ram_frequency')):
                return False
            if part1.get('form') != part2.get('ram_form'):
                return False
...
def set_comp_list2list_ex(partFullList1, partFullList2):
    """проверяет и устанавливает совместимость и список несовместимых элементов
     для ПЕРВОГО списка """
